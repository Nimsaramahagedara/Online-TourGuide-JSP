...
<body>
    <div class="navigation" style="margin: 0px;">
        ...
        <a href="AdminHandler?action=guiders" class="tab active"><i class="fa-solid fa-user-secret"></i> Guiders</a>
                ...
                <table class="table">
				  ...
				  <tbody>
					...
					<c:forEach var = "recT" items = "${users}">
						<tr>
				      		<th scope="row">${recT.id}</th>
				      		<td>${recT.name}</td>
				      		<td>${recT.email}</td>
				      		<td>${recT.phone}</td>
				      		<td>${recT.address}</td>
				      		<th scope="row"><a href="AdminHandler?action=deleteGuide&id=${recT.id}"> Delete </a></th>
